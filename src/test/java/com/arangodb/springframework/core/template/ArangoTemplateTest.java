...
public class ArangoTemplateTest extends AbstractArangoTest {
	...
	public void query() {
		...
		final ArangoCursor<Customer> cursor = template.query("FOR c IN @@coll FILTER c.`customerName` == @name RETURN c",
	...
	public void queryWithoutBindParams() {
		template.insert(new Customer("John", "Doe", 30));
		final ArangoCursor<Customer> cursor = template.query("FOR c IN `test-customer` FILTER c.`customerName` == 'John' RETURN c", null,
	...
	public void queryMap() {
		template.insert(new Customer("John", "Doe", 30));
		final ArangoCursor<Map> cursor = template.query("FOR c IN @@coll FILTER c.`customerName` == @name RETURN c",
		...
		assertThat(customers.size(), is(1));
		assertThat(customers.get(0).get("customerName"), is("John"));
	...
	public void queryBaseDocument() {
		template.insert(new Customer("John", "Doe", 30));
		final ArangoCursor<BaseDocument> cursor = template.query("FOR c IN @@coll FILTER c.`customerName` == @name RETURN c",
		...
		assertThat(customers.size(), is(1));
		assertThat(customers.get(0).getAttribute("customerName"), is("John"));
	...
	public void queryVPackSlice() {
		template.insert(new Customer("John", "Doe", 30));
		final ArangoCursor<VPackSlice> cursor = template.query("FOR c IN @@coll FILTER c.`customerName` == @name RETURN c",
			new MapBuilder().put("@coll", "test-customer").put("name", "John").get(), new AqlQueryOptions(),
			VPackSlice.class);
		assertThat(cursor, is(notNullValue()));
		final List<VPackSlice> customers = cursor.asListRemaining();
		assertThat(customers.size(), is(1));
		assertThat(customers.get(0).get("customerName").getAsString(), is("John"));
