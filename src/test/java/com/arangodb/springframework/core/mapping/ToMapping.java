 ...
 */
abstract class ToMapping extends AbstractMappingTxTestAbstract {

    ToMapping(boolean withinTx) {
        super(withinTx, ToMapping.class.getDeclaredClasses());
    }
    ...
    public void documentTo() {
        ...
        final DocumentToTestEntity document = template.find(e0.id, DocumentToTestEntity.class, findOpts).get();
    ...
    public void documentToLazy() {
        ...
        final BasicEdgeLazyTestEntity edge1 = new BasicEdgeLazyTestEntity(e1, e0);
        template.insert(edge1, insertOpts);
        final DocumentToLazyTestEntity document = template.find(e0.id, DocumentToLazyTestEntity.class, findOpts).get();
        assertThat(document, is(notNullValue()));
        assertThat(document.entities, is(notNullValue()));
        assertThat(document.entities.size(), is(2));
        for (final BasicEdgeLazyTestEntity e : document.entities) {
    ...
    public void constructorWithToParams() {
        ...
        final ConstructorWithToParamsTestEntity document = template
                .find(entity.id, ConstructorWithToParamsTestEntity.class, findOpts).get();
        assertThat(document, is(notNullValue()));
    ...
    public void constructorWithToLazyParams() {
        ...
        final ConstructorWithToLazyParamsTestEntity document = template
                .find(entity.id, ConstructorWithToLazyParamsTestEntity.class, findOpts).get();
        assertThat(document, is(notNullValue()));
        assertThat(document.value.stream().map((e) -> e.getId()).collect(Collectors.toList()),
                hasItems(edge1.id, edge2.id));
    }

    public static class SingleDocumentToTestEntity extends BasicTestEntity {
        @To
        private BasicEdgeLazyTestEntity entity;
    }

    @Test
    public void singleDocumentTo() {
        final SingleDocumentToTestEntity e0 = new SingleDocumentToTestEntity();
        template.insert(e0, insertOpts);
        final SingleDocumentToTestEntity e1 = new SingleDocumentToTestEntity();
        template.insert(e1, insertOpts);
        final BasicEdgeLazyTestEntity edge0 = new BasicEdgeLazyTestEntity(e1, e0);
        template.insert(edge0, insertOpts);
        final SingleDocumentToTestEntity document = template.find(e0.id, SingleDocumentToTestEntity.class, findOpts).get();
        assertThat(document, is(notNullValue()));
        assertThat(document.entity, is(notNullValue()));
        assertThat(document.entity.getId(), is(notNullValue()));
        assertThat(document.entity.getId(), is(edge0.getId()));
        assertThat(document.entity.getTo(), is(notNullValue()));
        assertThat(document.entity.getTo().getId(), is(notNullValue()));
        assertThat(document.entity.getTo().getId(), is(e0.getId()));
    }

    public static class SingleDocumentToLazyTestEntity extends BasicTestEntity {
        @To(lazy = true)
        private BasicEdgeLazyTestEntity entity;
    }

    @Test
    public void singleDocumentToLazy() {
        final SingleDocumentToLazyTestEntity e0 = new SingleDocumentToLazyTestEntity();
        template.insert(e0, insertOpts);
        final SingleDocumentToLazyTestEntity e1 = new SingleDocumentToLazyTestEntity();
        template.insert(e1, insertOpts);
        final BasicEdgeLazyTestEntity edge0 = new BasicEdgeLazyTestEntity(e1, e0);
        template.insert(edge0, insertOpts);
        final SingleDocumentToLazyTestEntity document = template.find(e0.id, SingleDocumentToLazyTestEntity.class, findOpts)
                .get();
        assertThat(document, is(notNullValue()));
        assertThat(document.entity, is(notNullValue()));
        assertThat(document.entity.getId(), is(notNullValue()));
