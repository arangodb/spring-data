...
public class SimpleArangoRepository<T> implements ArangoRepository<T> {
	 ...
	 *
	 * @param s represents either the key or id of a document to check for
	 ...
	 *
	 * @param strings an iterable with ids/keys of documents to get
	 ...
	 *
	 * @param s id or key of document to be deleted
	 */
	@Override
	...
	public void delete(final T entity) {
		String id = null;
		try {
			id = (String) arangoOperations.getConverter().getMappingContext().getPersistentEntity(domainClass).getIdProperty()
					.getField().get(entity);
	...
	private <S extends T> ArangoCursor<T> findAllInternal(final Sort sort, final Example<S> example,
	...
	private <S extends T> ArangoCursor<T> findAllInternal(final Pageable pageable, final Example<S> example,
			final Map<String, Object> bindVars) {

		final String query = String.format("FOR e IN %s %s %s RETURN e", getCollectionName(),
				buildFilterClause(example, bindVars), buildPageableClause(pageable, "e"));

		return arangoOperations.query(query, bindVars, pageable != null ? new AqlQueryOptions().fullCount(true) : null,
				domainClass);
	}
